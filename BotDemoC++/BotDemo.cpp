...
void OutputCommand(Object * myTeam);
	...
	while (gameTurn++ < maxTurn)
	{
		...
		for (int i = 0; i < N_PLAYER; i++)
		{
			if( stateMath != MATCH_PENALTY )
			{
				...
				{
					myTeam[i].m_action = Action::SHOOT;
				
					//Detect the rival goal 
					Position goalRivalPos = Position(mapWidth, mapHeight/2);
					if ((myTeamID == TEAM_ID_A && stateMath != HALF_1) || (myTeamID == TEAM_ID_B && stateMath == HALF_1)) goalRivalPos = Position(0, mapHeight / 2);
...
void OutputCommand(Object * myTeam){
	...
	printf("%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
		myTeam[0].m_action, myTeam[0].m_targetPos.x, myTeam[0].m_targetPos.y, myTeam[0].m_force,
		myTeam[1].m_action, myTeam[1].m_targetPos.x, myTeam[1].m_targetPos.y, myTeam[1].m_force,
		myTeam[0].m_action, myTeam[2].m_targetPos.x, myTeam[2].m_targetPos.y, myTeam[2].m_force,
		myTeam[0].m_action, myTeam[3].m_targetPos.x, myTeam[3].m_targetPos.y, myTeam[3].m_force,
		myTeam[0].m_action, myTeam[4].m_targetPos.x, myTeam[4].m_targetPos.y, myTeam[4].m_force);
}
