...
class Game {
    ...
    public void play() {
        ...
        final Position STRIKE_POINT = enemy.getGoal().getPos().getMoveToward(new Position(mapW / 2, mapH / 2), 1500);
        ...
        if(centerPoint.getState() == CenterPointState.A_POSSESS) {
            ...
            for(int j = 0; j < directions.size(); j++) {
                ...
                for(int i = 0; i < FORCES.length; i++) {
                    ...
                    newCenterPoints[i] = getBestCenterPoint(centerPoint.getTurn(), me, enemy, newBall, moveCap, shootCap, mapW, mapH, mainPlayer);
                }
            ...
            for(int j = 0; j < directions.size(); j++) {
                ...
                if(bestDestination == null || newCenterPoint.isBetter(nextCenterPoint)) {
                    bestDestination = destination;
                    bestForce = newForce;
                    nextCenterPoint = newCenterPoint;
                }
            }
            if(nextCenterPoint.getState() == CenterPointState.A_GOAL) {
                bestForce = 100;
            }
        }
        if(mainPlayer.getPos().getDist(ball.getPos()) <= shootCap) {
        	if(isEnemyShootable) {
        		Random random = new Random();
        		while(true) {
	        		Position randDir = directions.get(random.nextInt(directions.size()));
	        		if((long)randDir.getX() * (ball.getPos().getX() - me.getGoal().getPos().getX()) >= 0 &&
	        		   (long)randDir.getY() * (ball.getPos().getY() - me.getGoal().getPos().getY()) >= 0) {
	        			mainPlayer.getAction().shoot(ball.getPos().plus(randDir), 100);
	        			break;
	        		}
	        	}
        	}
        ...
        if(centerPoint.getState() == CenterPointState.A_POSSESS) {
            nextMainPlayer = nextCenterPoint.getPlayerA();
            mainCenterPoint = nextCenterPoint;
            if(nextMainPlayer != mainPlayer) {
                target[nextMainPlayer.getId()] = nextCenterPoint.getBall().getPos();
                nextMainPlayer.getAction().move(target[nextMainPlayer.getId()]);
            }
        }
        Player sweeper = null;
        Position sweepPoint = mainCenterPoint.getBall().getPos().getMoveToward(me.getGoal().getPos(), CENTER_POINT_RADIUS);
        ...
        if(centerPoint.getState() == CenterPointState.A_POSSESS) {
	        Player striker = null;
	        Position strikePoint = mainCenterPoint.getBall().getPos().getMoveToward(STRIKE_POINT, CENTER_POINT_RADIUS);
	    ...
	    while(true) {
            ...
            if(defender.getPos().getDist(ball.getPos()) <= shootCap && isEnemyShootable) {
                defender.getAction().shoot(ball.getPos().plus(ball.getPos().subtract(me.getGoal().getPos())), 100);
            }
            else {
