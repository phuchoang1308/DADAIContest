...
class Game {
    ...
    public void play() {
        final int SWEEP_1_RADIUS = 2000;
        final int SWEEP_2_RADIUS = 3500;
        final int SWEEP_3_RADIUS = 3500;
        final int MIDFIELD_RADIUS = 1000;
        final int STRIKE_RADIUS = 1000;
        final int DIRECTION_RADIUS = 4;
        final int MARK_THRESHOLD = 20000;
        final int FRONT_MARK_RADIUS = 300;
        final int BACK_MARK_RADIUS = 500;
        ...
        if(centerPoint.getState() == CenterPointState.A_POSSESS) {
            ...
            for(int i = 0; i < directions.size(); i++) {
                ...
                for(int j = 0; j < FORCES.length; j++) {
                    ...
                    newCenterPoints[i][j] = getBestCenterPoint(centerPoint.getTurn(), me, enemy, newBall, moveCap, shootCap, mapW, mapH, centerPoint.getPlayerA());
                }
            }
            for(int i = 0; i < directions.size(); i++) {
                Position direction = directions.get(i);
        ...
        Position sweepPoint3 = centerPoint.getBall().getPos().getMoveToward(SWEEP_POINT, SWEEP_3_RADIUS);
        Position strikePoint = centerPoint.getBall().getPos().getMoveToward(STRIKE_POINT, STRIKE_RADIUS);
        Position midfieldPoint = centerPoint.getBall().getPos().getMoveToward(getMiddlePoint(centerPoint.getBall().getPos(), SWEEP_POINT, MIDFIELD_RADIUS, STRIKE_POINT, MIDFIELD_RADIUS), MIDFIELD_RADIUS);
        if(centerPoint.getState() == CenterPointState.A_GOAL) {
            ...
            midfieldPoint = subCenterPoint.getBall().getPos().getMoveToward(getMiddlePoint(subCenterPoint.getBall().getPos(), SWEEP_POINT, MIDFIELD_RADIUS, STRIKE_POINT, MIDFIELD_RADIUS), MIDFIELD_RADIUS);
            mainPlayer = findMainPlayer(target, centerPoint, ball, bestDestination, bestForce, isEnemyShootable);
        ...
        else if(centerPoint.getState() == CenterPointState.CLASH) {
            mainPlayer = findMainPlayer(target, centerPoint, ball, bestDestination, bestForce, isEnemyShootable);
            if(me.getScore() <= enemy.getScore()) {
                nextMainPlayer = findMainPlayer(target, centerPoint, ball, bestDestination, bestForce, isEnemyShootable);
            }
            markedPlayer[centerPoint.getPlayerB().getId()] = true;
            findDefenders(target, centerPoint, ball, markedPlayer, MARK_THRESHOLD, FRONT_MARK_RADIUS, BACK_MARK_RADIUS, isEnemyShootable);
        }
        else {
        	mainPlayer = findMainPlayer(target, centerPoint, ball, bestDestination, bestForce, isEnemyShootable);
            markedPlayer[centerPoint.getPlayerB().getId()] = true;
    ...
    private void findDefenders(Position[] target, CenterPoint centerPoint, Ball ball, boolean[] markedPlayer, int markThreshold, int frontMarkRadius, int backMarkRadius, boolean isEnemyShootable) {
        while(true) {
            ...
            target[defender.getId()] = getMiddlePoint(mainThreat.getPos(), centerPoint.getBall().getPos(), frontMarkRadius, me.getGoal().getPos(), backMarkRadius);
            if(defender.getPos().getDist(ball.getPos()) <= shootCap && isEnemyShootable) {
                defender.getAction().shoot(new Position(centerPoint.getTeamB().getGoal().getPos().getX(), ball.getPos().getY()), 100);
            }
            else {
    ...
    List<List<Integer>> genPerms(int n) {
        List<Integer> perm = new ArrayList<>();
        List<List<Integer>> perms = new ArrayList<>();
        Set<Integer> numSet = new HashSet<>();
        recursiveGenPerms(n, 0, numSet, perm, perms);
        return perms;
    }

    void recursiveGenPerms(int n, int pos, Set<Integer> numSet, List<Integer> perm, List<List<Integer>> perms) {
        if(pos == n) {
        ...
        for(int i = 0; i < n; i++) {
            if(!numSet.contains(i)) {
                numSet.add(i);
                perm.add(i);
                recursiveGenPerms(n, pos + 1, numSet, perm, perms);
    ...
    Position getMiddlePoint(Position o, Position x, double ox, Position y, double oy) {
    	Position xx = o.getMoveOver(x, ox);
    	Position yy = o.getMoveOver(y, oy);
    	return o.plus(xx.subtract(o)).plus(yy.subtract(o));
    }
}
